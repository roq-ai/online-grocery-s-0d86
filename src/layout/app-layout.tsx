...
import { useSupermarketFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['owner', 'manager', 'employee'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSupermarketFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Supermarkets',
      path: isTenantUser ? `/supermarkets/view/${data?.id}` : '/supermarkets',
      entity: 'supermarket',
