...
import { useRoqClient, useSupermarketFindManyWithCount } from 'lib/roq';
...
export function SupermarketListPage(props: SupermarketListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'inventory.count', 'order.count', 'product.count'],
  ...
  const { data, error, isLoading, mutate } = useSupermarketFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.supermarket.delete({ where: { id } });
      await mutate();
    } catch (error) {
